...
Jump to your option:
- [Option A — uvx (recommended)](#option-a--uvx-recommended)
...
No cloning, no virtualenv, no `pip install`. Add the config below to your MCP client and it works on first run — `uvx` downloads and caches keila-mcp automatically.

Skip to [**Client Configuration**](#client-configuration) and use the **uvx** config for your client.

---

## **Option B — Local install**

Use this if you need offline access, want to modify the code, or contribute to the project.
...
Copy the path printed by `pwd` — you will need it in the local install configs below.
...
Copy the path printed by `cd` — you will need it in the local install configs below.
...
Each client section below shows **two configs — pick the one matching your install choice.**

> [!IMPORTANT]
> **Local install users:** replace `/path/to/keila-mcp` with the path printed by `pwd` (or `cd` on Windows) at the end of the installation section above.
...
# local install — stdio mode (default)
python -m keila_mcp.mcp_server

# local install — HTTP mode
